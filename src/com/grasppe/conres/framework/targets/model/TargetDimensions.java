...
public class TargetDimensions extends AbstractModel {
    ...
    float[]	xCenters,	// = new float[] {2.3F, 8.1F, 14F, 19.8F, 25.7F, 31.5F, 37.4F, 43.2F, 49.1F, 54.9F, 60.7F, 66.5F, 72.3F, 78.1F },
			yCenters;	// = new float[] {2.3F, 6.8F, 11.3F, 15.8F, 20.3F, 24.8F, 29.3F, 33.8F, 38.3F, 42.8F, 47.3F, 51.8F, 56.3F, 60.8F};
    
    float	xRepeat,	// = xCenters[1] - xCenters[0],
			yRepeat;	// = yCenters[1] - yCenters[0];
    int		xCount,		//  = xCenters.length,
			yCount; 	//  = yCenters.length;
    float[]	xBounds,	// = new float[] { 0.0F, xCenters[xCount - 1] + xRepeat - xSpan },		// + xCenters[1] - xCenters[0] - xSpan };
			yBounds;	// = new float[] { 0.0F, yCenters[yCount - 1] + yRepeat - ySpan };		// + yCenters[1] - yCenters[0] - ySpan };
...
//      xRepeat = getXCenters()[2] - getXCenters()[1];
...
//      yRepeat = getYCenters()[2] - getYCenters()[1];
     ...
     *  @param ySpan
     */
    private void setDimensions(float[] xCenters, float[] yCenters, float xSpan, float ySpan) {
...
//      boolean   tSizeValid = (xBounds.length <= 2);
        ...
        if (xBounds.length == 0)
            xBounds = new float[] { xCenters[nX - 1] - (xSpan / 2.0F) + xCenters[2] - xCenters[1] };
        if (yBounds.length == 0)
            yBounds = new float[] { yCenters[nY - 1] - (ySpan / 2.0F) + yCenters[2] - yCenters[1] };
