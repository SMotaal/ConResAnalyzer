...
//private int               minColumn   = -1;
  ...
  protected boolean checkAbove(int row) {
    PatchDesignation alternate = ASSUMED_PASS;
    ...
    for (int r = row - 1; r >= getMinRow(); r--) {
      ...
      if (((current != ASSUMED_MARGINAL) || (current != expecting)) && (current != alternate) && (!isNotAssumed(current))) return false;
  ...
  protected boolean checkBelow(int row) {
    PatchDesignation alternate = ASSUMED_FAIL;
    PatchDesignation current   = finalState.getDesignation(row, column);
    PatchDesignation expecting = current;
    ...
    for (int r = row + 1; r <= getMaxRow(); r++) {
      ...
      if (((current != ASSUMED_MARGINAL) || (current != expecting)) && (current != alternate)) return false;
   ...
   *  @return
   */
  public static BlockState fillValues(BlockState blockState, int startRow, int endRow, int column, PatchDesignation newValue) {
   ...
   *  @return
   */
  public static BlockState fillValues(BlockState blockState, int startRow, int endRow, int column, PatchDesignation testValue, PatchDesignation newValue) {
    if (startRow < endRow) {
      for (int r = startRow; r <= endRow; r++) {
        if ((testValue == null) || (blockState.getDesignation(r, column) != testValue)) blockState.setValue(newValue, r, column);
      }
   ...
   */
  public static int findNextIs(BlockState blockState, int row, int column, PatchDesignation designation) {
    int     maxRow = blockState.rows - 1;
    boolean valid  = false;

    while ((row <= maxRow)
           && (valid = isNotSame(blockState.getDesignation(row, column),
                                 designation)))			// blockState.getDesignation(row, column) != designation))
      row++;
   ...
   *  @return last row below row with designation or -1 if not found
   */
  public static int findNextNot(BlockState blockState, int row, int column, PatchDesignation designation) {
    int     maxRow = blockState.rows - 1;
    boolean valid  = false;

    while ((row <= maxRow)
           && (valid = isSame(blockState.getDesignation(row, column),
                              designation)))		// blockState.getDesignation(row, column) == designation))
      row++;
   ...
   */
  public static int findPastIs(BlockState blockState, int row, int column, PatchDesignation designation) {
    int     minRow = 0;
    boolean valid  = false;
   ...
   *  @return first row above row with designation or -1 if not found
   */
  public static int findPastNot(BlockState blockState, int row, int column, PatchDesignation designation) {
    int     minRow = 0;
    boolean valid  = false;
...
// *  @return first row above row with designation or -1 if not found
// */
//public static int findSecondLastIs(BlockState blockState, int row, int column, PatchDesignation designation) {
...
// *  @return first row above row with designation or -1 if not found
// */
//public static int findSecondNextIs(BlockState blockState, int row, int column, PatchDesignation designation) {
...
//}
  ...
  protected boolean moveBy(int rowSteps, int columnSteps) {
    return moveTo(getFinalState().getRow() + rowSteps, getFinalState().getColumn() + columnSteps);
  }
...
//  int   lastMarginal = findPastIs(finalState, getMaxRow(), column, MARGINAL);
  ...
  /**
   *    @param patchDesignation
   *    @param testDesignation
   *    @return
   */
  private static boolean isNotSame(PatchDesignation patchDesignation, PatchDesignation testDesignation) {
    return ((patchDesignation != testDesignation) && !isNotAssumed(patchDesignation));
  }
   ...
   *    @return
   */
  private static boolean isSame(PatchDesignation patchDesignation, PatchDesignation testDesignation) {
    return ((patchDesignation == testDesignation) || isNotAssumed(patchDesignation));
  }
  
  private static boolean isNotAssumed(PatchDesignation patchDesignation) {
	  return (patchDesignation == PASS) || (patchDesignation == FAIL)
	            || (patchDesignation == MARGINAL);
  }
