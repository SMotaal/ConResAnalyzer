...
//    resizedImage = new BufferedImage(imageWidth, imageHeight, getBlockMapImage().getType());
//            
//            resizedImage.setRenderingHint(RenderingHints.KEY_INTERPOLATION,RenderingHints.VALUE_INTERPOLATION_BILINEAR);
//            resizedImage.setRenderingHint(RenderingHints.KEY_RENDERING,RenderingHints.VALUE_RENDER_QUALITY);
//            resizedImage.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);

    	  BufferedImage blockImage = getBlockMapImage();
      g.drawImage(resizeBlockImage(blockImage), imageLeft, imageTop, imageWidth, imageHeight, this);
      }
    } catch (Exception exception) {
      GrasppeKit.debugError("Painting Block Tags", exception, 2);
    }
   ...
   * 	@return
   */
  private BufferedImage resizeBlockImage(BufferedImage originalImage) {
...
//      imageLength = Math.max(imageLength, maximumWidth);
        ...
        setMinimumSize(new Dimension(imageWidth, imageHeight));
        newScale = Math.max(minimumScale, scaleValue(blockMapWidth, imageWidth, blockMapHeight, imageHeight));
      }

      imageScale = newScale;

      if (horizontalAlignment == LEFT) imageLeft = 0;
      else if (horizontalAlignment == CENTER) imageLeft = (int)Math.round((getWidth() - imageWidth) / 2.0);
      else if (horizontalAlignment == RIGHT) imageLeft = getWidth() - imageWidth;

      if (verticalAlignment == TOP) imageTop = 0;
      else if (verticalAlignment == CENTER) imageTop = (int)Math.round((getHeight() - imageHeight) / 2.0);
  ...
  protected int getMaxmimumBlockDimension() {
    return Math.max(getModel().getBlockState().getColumns(), getModel().getBlockState().getRows());
  }
