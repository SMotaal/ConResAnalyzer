...
//protected boolean                                 detachable = true;
  protected LinkedHashMap<String, AbstractCommand> commands; // = new LinkedHashMap<String, AbstractCommand>();
...
//  if (actionListener != null) actionListener.actionPerformed(e);        // Pass up the chain of responsibility
   ...
   */
  public LinkedHashMap<String, AbstractCommand> appendCommands(LinkedHashMap<String,
                       AbstractCommand> allCommands, LinkedHashMap<String, AbstractCommand> newCommands) {
    if (allCommands == null) allCommands = new LinkedHashMap<String, AbstractCommand>();
    if (allCommands.isEmpty()) if ((commands != null) &&!commands.isEmpty()) allCommands.putAll(commands);
...
//  notify();
    ...
    try {
      ...
      if ((commands != null) && (commands.size() > 0)) {
        for (String key : commands.keySet()) {
          GrasppeKit.debugText("Detatching", commands.get(key).getClass().getSimpleName(), 1);
          commands.get(key).detatch();
        }
      }
      ...
      if (getModel() != null) {
        ...
        for (int i = 0; i < views.length; i++) {
          GrasppeKit.debugText("Detatching", views[i].getClass().getSimpleName(), 1);
          views[i].detatch();
        }
      ...
      GrasppeKit.debugError("Detatching Controller", exception, 1);
    }
  ...
  public void finalize() throws Throwable {
    GrasppeKit.debugText("Finalizing", getClass().getSimpleName(), 5);
