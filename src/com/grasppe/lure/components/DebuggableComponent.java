/*
 * @(#)DebuggableComponent.java   12/12/04
 * 
...
public abstract class DebuggableComponent extends AbstractComponent {
  ...
  protected static boolean          debuggingEnabled    = false;
  ...
  protected void addDebugView(JPanel panel) {
    ...
    EmptyBorder border = new EmptyBorder(debugPadding, debugPadding, debugPadding, debugPadding);
  ...
  private void updateDebugFrame() {
    ...
    if (!isDebuggingEnabled() && debugViewFrame.isVisible()) debugViewFrame.setVisible(false);
    if (!isDebuggingEnabled() || GrasppeKit.isRunningJar()) return;
   ...
   *  @param falseText
   */
  protected void updateDebugLabel(String name, boolean expression, String trueText, String falseText) {
  ...
  protected void updateDebugLabels() {
    ...
    try {
      debugTitleLabel.setText("<html><b>" + getClass().getSimpleName() + "</b>&nbsp;(" + getModel().observerString()
                              + ")</html>");
    } catch (Exception exception) {
      debugTitleLabel.setText("<html><b>" + getClass().getSimpleName() + "</b>&nbsp;(?)</html>");
      GrasppeKit.debugError("DebugView", exception, dbg);
    }
  ...
  public static void setDebuggingEnabled(boolean debuggingEnabled) {
    DebuggableComponent.debuggingEnabled = debuggingEnabled;
  }
}
