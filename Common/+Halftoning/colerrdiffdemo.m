function colerrdiffdemo(action,varargin);

% colerrdiffdemo Separable Error Diffusion Halftoning Demo
%                  
%   This Demo lets you design digital color halftones by
%   Separable Error Diffusion. This is equivalent to applying
%   grayscale error diffusion halftoning methods separably to 
%   each of the 3 color planes. The quality of the halftone is  
...
%   The halftone image file is saved by the name col_errdiff_halftone.tif 
...
% If colerrdiffdemo is already running, bring it to the foreground.
h = findobj(allchild(0), 'tag', 'Separable Color Error Diffusion Halftoning');
if ~isempty(h)
   figure(h(1))
   return
end
...
FirDemoFig = figure( ...
   'Name','Separable Color Error Diffusion Halftoning', ...
   'NumberTitle','off', 'HandleVisibility', 'on', ...
   'tag', 'Separable Color Error Diffusion Halftoning', ...
...
callbackStr='colerrdiffdemo(''SaveHalftone'');';
...
h = uicontrol('Parent', FirDemoFig, ...
   ...
   callBackStr = 'colerrdiffdemo(''setUpMethod'')';
...
hdl.ImgPop=uicontrol('Parent', FirDemoFig, ...
   ...
   'Tag','ImagesPop',...
   'Callback','colerrdiffdemo(''LoadNewImage'')');
...
labelStr='Raster';
callbackStr='colerrdiffdemo(''RadioUPDATE'',1);';
...
yPos = btnTop-(btnNumber-1)*(radioHt+spacing);
labelStr='Serpentine';
callbackStr='colerrdiffdemo(''RadioUPDATE'',2);';
...
callbackStr='colerrdiffdemo(''GenerateHalftone'')';
...
% The INFO button
labelStr='Info';
callbackStr='helpwin colerrdiffdemo';
...
case 3
       ...
       OUT(:,:,3) = OUT3;
case 4       
       OUT1 = floyd_serp(inp(:,:,1));
       OUT2 = floyd_serp(inp(:,:,2));
       OUT3 = floyd_serp(inp(:,:,3));
       OUT(:,:,1) = OUT1;
       OUT(:,:,2) = OUT2;
       OUT(:,:,3) = OUT3;
case 5       
       OUT1 = jarvis_serp(inp(:,:,1));
       OUT2 = jarvis_serp(inp(:,:,2));
       OUT3 = jarvis_serp(inp(:,:,3));
       OUT(:,:,1) = OUT1;
       OUT(:,:,2) = OUT2;
       OUT(:,:,3) = OUT3;
case 6       
       OUT1 = stucki_serp(inp(:,:,1));
       OUT2 = stucki_serp(inp(:,:,2));
       OUT3 = stucki_serp(inp(:,:,3));
       OUT(:,:,1) = OUT1;
       OUT(:,:,2) = OUT2;
       OUT(:,:,3) = OUT3;
...
col_errdiff_halftone = get(hdl.FilteredImage,'Cdata');
imwrite(col_errdiff_halftone,'col_errdiff_halftone.tif');
