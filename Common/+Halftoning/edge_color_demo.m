function edge_color_demo(action,varargin);

% EDGEENHANCEDEMO Edge Enhancement Color Error Diffusion Halftoning Demo
%                  
%   This Demo lets you design digital color halftones by Edge Enhancement
%   error Diffusion. The quality of the halftone is  
%   evaluated on the scale of various Image Quality Metrics.
%   The color halftones are generated by applying Edge Enhancement
...
% If edge_color_demo is already running, bring it to the foreground.
...
callbackStr='edge_color_demo(''SaveHalftone'');';
...
h = uicontrol('Parent', FirDemoFig, ...
   ...
   callBackStr = 'edge_color_demo(''setUpMethod'');';
...
hdl.ImgPop=uicontrol('Parent', FirDemoFig, ...
   ...
   'Tag','ImagesPop',...
   'Callback','edge_color_demo(''LoadNewImage'');');
...
callbackStr='edge_color_demo(''setUpEdge'');';
...
callbackStr='edge_color_demo(''RadioUPDATE1'',1);';
...
callbackStr='edge_color_demo(''RadioUPDATE1'',2);';
...
yPos = btnTop-(btnNumber-1)*(radioHt+spacing);
labelStr='Raster';
callbackStr='edge_color_demo(''RadioUPDATE'',1);';
...
yPos = btnTop-(btnNumber-1)*(radioHt+spacing);
labelStr='Serpentine';
callbackStr='edge_color_demo(''RadioUPDATE'',2);';
...
callbackStr='edge_color_demo(''GenerateHalftone'');';
...
% The INFO button
labelStr='Info';
callbackStr='helpwin edge_color_demo';
...
imwrite(edge_halftone,'edge_color_halftone.tif');
...
case 1
     ...
     if (dbf == 0)
     [OUT(:,:,1),qn,k] = edgerrfixed(inp1/255,fcfixed,sp,dir,v);
     [OUT(:,:,2),qn,k] = edgerrfixed(inp2/255,fcfixed,sp,dir,v);
     [OUT(:,:,3),qn,k] = edgerrfixed(inp3/255,fcfixed,sp,dir,v);
     end
     
     if (dbf == 1)
     [OUT(:,:,1),qn,L_adap] = edgerrdiff_l(inp1,fc,sp,dir,v,0,nu1);    
     [OUT(:,:,2),qn,L_adap] = edgerrdiff_l(inp2,fc,sp,dir,v,0,nu1);    
     [OUT(:,:,3),qn,L_adap] = edgerrdiff_l(inp3,fc,sp,dir,v,0,nu1);    
     OUT = ((OUT +1)/2);   
     end
...
case 2
     ...
     if (dbf == 1)
     [OUT(:,:,1),qn,L_adap,fc,u,K] = edgerrdiff_l_h(inp1,fc,sp,dir,v,dbf-1);
     [OUT(:,:,2),qn,L_adap,fc,u,K] = edgerrdiff_l_h(inp2,fc,sp,dir,v,dbf-1);
     [OUT(:,:,3),qn,L_adap,fc,u,K] = edgerrdiff_l_h(inp3,fc,sp,dir,v,dbf-1);
     OUT = ((OUT +1)/2);        
     end     

     if (dbf == 0)
     [OUT(:,:,1),qn,L_adap,fc,u,K] = edgerrdiff_l_h(inp1,fc,sp,dir,v,-0.1);
     [OUT(:,:,2),qn,L_adap,fc,u,K] = edgerrdiff_l_h(inp2,fc,sp,dir,v,-0.1);
     [OUT(:,:,3),qn,L_adap,fc,u,K] = edgerrdiff_l_h(inp3,fc,sp,dir,v,-0.1);
     OUT = ((OUT +1)/2);        
     end     
