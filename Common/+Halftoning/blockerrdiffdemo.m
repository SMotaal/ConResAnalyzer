function blockerrdiffdemo(action,varargin);

% BLOCKERRDIFFDEMO Block Error Diffusion Halftoning Demo
%                  
%   This Demo lets you design digital grayscale halftones by Block
%   Error Diffusion. Block Error Diffusion halftones are 
%   of the greatest importance in robust printing and on printing with
%   printers that suffer from 'dot spread'
%
...
%   The halftone image file is saved by the name block_halftone.tif 
...
%   The pixel-block size (rectangular) may be selected by specifying the
%   number of rows and columns. The mask should be of the size rows x columns
%   and should only contain the entries 0 and 1, the pixels in the mask
%   selected to be 1 form a part of the dot shape.
...
%%%
%%%  Sub-function - InitializeBlock Error Diffusion Demo
%%%

function InitializeHALFTONINGDEMO1()

% If blockerrdiffdemo is already running, bring it to the foreground.
h = findobj(allchild(0), 'tag', 'FM Halftones of User Controlled Dot shapes and Sharpness via Block Error Diffusion');
if ~isempty(h)
   figure(h(1))
   return
end
...
FirDemoFig = figure( ...
   'Name','FM Halftones of User Controlled Dot shapes and Sharpness via Block Error Diffusion', ...
   'NumberTitle','off', 'HandleVisibility', 'on', ...
   'tag', 'FM Halftones of User Controlled Dot shapes and Sharpness via Block Error Diffusion', ...
...
callbackStr='blockerrdiffdemo(''SaveHalftone'');';
...
h = uicontrol('Parent', FirDemoFig, ...
   ...
   callBackStr = 'blockerrdiffdemo(''setUpMethod'');';
...
hdl.ImgPop=uicontrol('Parent', FirDemoFig, ...
   ...
   'Tag','ImagesPop',...
   'Callback','blockerrdiffdemo(''LoadNewImage'');');
...
%====================================
% Row and Column Entries for Block Size
menuNumber=2;
yPos=menutop-(menuNumber-1)*(btnHt+spacing)-1.4*txtsp;
callbackStr='blockerrdiffdemo(''setUpBlock'');';
 
labelPos = [left yPos textWidth textHeight];
h = uicontrol('Parent', FirDemoFig, ...
   'Style','text', ...
   'Units','normalized', ...
   'Position',labelPos, ...
   'Horiz','left', ...
   'String','Enter pixel-block size', ...
   'Interruptible','off', ...
   'BackgroundColor',bgc, ...
   'ForegroundColor','white');

yPos = yPos - btnHt/2;
labelPos = [left yPos textWidth/2 textHeight];
h = uicontrol('Parent', FirDemoFig, ...
   'Style','text', ...
   'Units','normalized', ...
   'Position',labelPos, ...
...
labelPos = [left+textWidth/2+frmBorder  yPos textWidth/2 textHeight];
h = uicontrol('Parent', FirDemoFig, ...
   'Style','text', ...
   'Units','normalized', ...
   'Position',labelPos, ...
...
yPos = yPos - btnHt/2;
% Text field
labelRows = yPos - textHeight;
textPos = [left yPos textWidth/2 textHeight];
callbackStr = 'blockerrdiffdemo(''textBlockRows'');';
hdl.Rows = uicontrol('Parent', FirDemoFig, ...
   'BusyAction','Queue','Interruptible','off',...
   'Style','edit', ...
   'Units','normalized', ...
   'Position',textPos, ...
...
textPos = [left+textWidth/2+frmBorder yPos textWidth/2 textHeight];
callbackStr = 'blockerrdiffdemo(''textBlockColumns'');';
hdl.Columns = uicontrol('Parent', FirDemoFig, ...
   'BusyAction','Queue','Interruptible','off',...
   'Style','edit', ...
   'Units','normalized', ...
   'Position',textPos, ...
...
%===================================
% textBox for Specifying Dot Shape Mask

yPos = yPos - .08; 
labelWidth = frmWidth-textWidth-.01;
labelLeft = left;
labelPos = [left yPos  1.2*textWidth txtsp];
 h = uicontrol('Parent', FirDemoFig, ...
    'Style','text', ...
   'Units','normalized', ...
   'Position',labelPos, ...
   'Horiz','left', ...
   'String','Specify Mask for Dot shape - default is a 3 x 3 mask for "plus shape" dots', ...
   'Interruptible','off', ...
   'BackgroundColor',bgc, ...
   'ForegroundColor','white');
% Text field
labelBottom = yPos - textHeight;
textPos = [labelLeft labelBottom 1.3*textWidth textHeight];
callbackStr = 'blockerrdiffdemo(''textBlockMask'');';
hdl.Mask = uicontrol('Parent', FirDemoFig, ...
   'BusyAction','Queue','Interruptible','off',...
   'Style','edit', ...
   'Units','normalized', ...
   'Position',textPos, ...
   ...
   'Background','white', ...
   'Foreground','black', ...
   'String','[0 1 0; 1 1 1; 0 1 0]','Userdata',[0 1 0; 1 1 1; 0 1 0], ...
   'callback',callbackStr);
...
labelStr='Raster';
callbackStr='blockerrdiffdemo(''RadioUPDATE'',1);';
...
yPos = btnTop-(btnNumber-1)*(radioHt+spacing);
labelStr='Serpentine';
callbackStr='blockerrdiffdemo(''RadioUPDATE'',2);';
...
callbackStr='blockerrdiffdemo(''GenerateHalftone'');';
...
%%%%%%%%%Error filter Message%%%%%%%%%%%%%%%
 ...
 h = uicontrol('Parent', FirDemoFig, ...
   ...
   'Position',messagePos, ...
   'Horiz','left', ...
   'String','Entries in the Mask control block shape',... 
   'Interruptible','off', ...
   'BackgroundColor',[0.8 0.8 0.8], ...
   'ForegroundColor','red');
...
% The INFO button
labelStr='Info';
callbackStr='helpwin blockerrdiffdemo';
...
function textBlockRows(DemoFig)
...
v = get(hdl.Rows,'Userdata');
s = get(hdl.Rows,'String');
vv = eval(s,num2str(v));
if isempty(vv) | ~isreal(vv)
     vv = v; 
     set(hdl.Rows, 'String', num2str(v))
    return
end

res = sprintf('%s', num2str(vv)');

set(hdl.Rows,'Userdata',vv,'String',res);

setUpBlock;
return

function textBlockColumns(DemoFig)
...
hdl = get(DemoFig,'Userdata');
v = get(hdl.Columns,'Userdata');
s = get(hdl.Columns,'String');
vv = eval(s,num2str(v));
if isempty(vv) | ~isreal(vv)
     vv = v; 
     set(hdl.Columns, 'String', num2str(v))
    return
end

res = sprintf('%s', num2str(vv)');

set(hdl.Columns,'Userdata',vv,'String',res);

setUpBlock;
return

function textBlockMask(DemoFig)
...
hdl = get(DemoFig,'Userdata');
v = get(hdl.Mask,'Userdata');
s = get(hdl.Mask,'String');
vv = eval(s,num2str(v));
if isempty(vv) | ~isreal(vv)
     vv = v; 
     set(hdl.Mask, 'String', num2str(v))
    return
end

res = sprintf('%s', num2str(vv)');

set(hdl.Mask,'Userdata',vv,'String',res);
...
block_halftone = get(hdl.FilteredImage,'Cdata');
imwrite(block_halftone,'block_halftone.tif');
...
mask1 = get(hdl.Mask,'Userdata'); % Get Mask for Block Shape
mask0 = 1 - mask1;

sp = get(hdl.Slider,'value'); % Sharpness from the slider [0,1]

N(1) = get(hdl.Rows,'Userdata');   %  Extract Rows for Block Size
N(2) = get(hdl.Columns,'Userdata'); %  Extract Columns for Block Size

OUT = bed(inp,N,mask1,mask0,sp);
...
otherwise 
   error('Block Error Diffusion DEMO: Unknown Image Option!');
end
