...
function setDesktopStatus(statusText)
  try
    ...
    %         try
    try
      grasppeObject = get(get(0, 'CurrentFigure'), 'UserData');
      if isa(grasppeObject, 'Grasppe.Core.Prototype')
        statusbar(grasppeObject.Handle, statusText);
        refresh(grasppeObject.Handle);
        try desktop.setStatusText(''); end
        try stop(timerfind('Tag', 'rootStatusbarTimer')); end
        return;
      end
    end
    ...
    if isempty(statusText)
      try
        stop(timerfind('Tag', 'rootStatusbarTimer'));
        desktop.setStatusText('');
      end
    else
      try
        stop(timerfind('Tag', 'rootStatusbarTimer'));
      ...
      try
        t = timer('Tag','rootStatusbarTimer', 'TimerFcn',{@setText,desktop,statusText}, 'StartDelay', 0.05, 'Period', 0.25, 'ExecutionMode','fixedDelay');
        start(t);
      end
    end
    %             tmr = timer('Name','rootStatusbarTimer', 'TimerFcn',{@setText,desktop,statusText}, 'StartDelay',0.05, 'ExecutionMode','fixedDelay');
    %             start(tmr);
    %         catch
