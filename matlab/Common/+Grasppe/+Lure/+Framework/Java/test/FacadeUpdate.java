...
public class FacadeUpdate implements Observable, Observer
{
  private java.util.Vector data = new java.util.Vector();
  public synchronized void addMyUpdateListener(MyUpdateListener lis) {
    data.addElement(lis);
  }
  public synchronized void removeMyUpdateListener(MyUpdateListener lis) {
    data.removeElement(lis);
  }
  public interface MyUpdateListener extends java.util.EventListener {
    void updateEvent(MyUpdateEvent event);
  }
  public class MyUpdateEvent extends java.util.EventObject {
    private static final long serialVersionUID = 1L;
    public float oldValue,newValue;
    MyUpdateEvent(Object obj, float oldValue, float newValue) {
      super(obj);
  ...
  public void notifyMyUpdate() {
    java.util.Vector dataCopy;
    synchronized(this) {
      dataCopy = (java.util.Vector)data.clone();
    }
    for (int i=0; i<dataCopy.size(); i++) {
      MyUpdateEvent event = new MyUpdateEvent(this, 0, 1);
      ((MyUpdateListener)dataCopy.elementAt(i)).updateEvent(event);
    }
  }
